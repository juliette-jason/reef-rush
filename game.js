...
const MERMAID_LOOKS = [
  {
    ...
    skin: "#f8dcc4",
    skinShade: "#e8b896",
    hair: ["#fb923c", "#ea580c", "#c2410c", "#7c2d12"],
    hairHi: "#fdba74",
    shell: ["#f0abfc", "#c084fc", "#a855f7", "#7e22ce"],
    shellHi: "#fce7f3",
    clasp: "#e879f9",
    tail: ["#6ee7b7", "#34d399", "#059669", "#064e3b"],
    scaleStroke: "rgba(167, 243, 208, 0.5)",
    lips: "#db2777",
  },
  {
    id: "pearl",
    skin: "#f5e0d0",
    skinShade: "#e0b8a0",
    hair: ["#fef3c7", "#fcd34d", "#f59e0b", "#b45309"],
    hairHi: "#fef9c3",
    shell: ["#fda4af", "#fb7185", "#e11d48", "#9f1239"],
    shellHi: "#ffe4e6",
    clasp: "#fb7185",
    tail: ["#7dd3fc", "#38bdf8", "#0284c7", "#0c4a6e"],
    scaleStroke: "rgba(186, 230, 253, 0.5)",
    lips: "#e11d48",
  },
  {
    id: "laguna",
    skin: "#e8c4a8",
    skinShade: "#c99572",
    hair: ["#1e1b4b", "#312e81", "#4338ca", "#1e1b4b"],
    hairHi: "#818cf8",
    shell: ["#fde68a", "#fbbf24", "#d97706", "#92400e"],
    shellHi: "#fef3c7",
    clasp: "#f59e0b",
    tail: ["#c4b5fd", "#a78bfa", "#7c3aed", "#4c1d95"],
    scaleStroke: "rgba(221, 214, 254, 0.5)",
    lips: "#be185d",
  },
  {
    id: "coral",
    skin: "#f3d5b5",
    skinShade: "#d4a574",
    hair: ["#7f1d1d", "#991b1b", "#b91c1c", "#450a0a"],
    hairHi: "#fca5a5",
    shell: ["#99f6e4", "#2dd4bf", "#0d9488", "#115e59"],
    shellHi: "#ccfbf1",
    clasp: "#14b8a6",
    tail: ["#f9a8d4", "#ec4899", "#be185d", "#831843"],
    scaleStroke: "rgba(251, 207, 232, 0.5)",
    lips: "#9d174d",
  },
  {
    id: "tide",
    skin: "#f6e7d8",
    skinShade: "#e2c4a8",
    hair: ["#0f766e", "#115e59", "#134e4a", "#042f2e"],
    hairHi: "#5eead4",
    shell: ["#e9d5ff", "#d8b4fe", "#a855f7", "#6b21a8"],
    shellHi: "#f3e8ff",
    clasp: "#c084fc",
    tail: ["#fde047", "#eab308", "#ca8a04", "#713f12"],
    scaleStroke: "rgba(254, 240, 138, 0.45)",
    lips: "#c026d3",
  },
  {
    id: "moonfin",
    skin: "#ead7c4",
    skinShade: "#c9a88a",
    hair: ["#e2e8f0", "#cbd5e1", "#94a3b8", "#475569"],
    hairHi: "#f8fafc",
    shell: ["#bfdbfe", "#60a5fa", "#2563eb", "#1e3a8a"],
    shellHi: "#dbeafe",
    clasp: "#93c5fd",
    tail: ["#86efac", "#4ade80", "#16a34a", "#14532d"],
    scaleStroke: "rgba(187, 247, 208, 0.5)",
    lips: "#db2777",
  },
];

/**
 * Draw one mermaid in a horizontal dolphin-kick swim (head +X, fluke −X).
 * `kick` drives the undulating tail; caller sets translate / facing / alpha.
 */
function drawSwimmingMermaidFigure(s, look, kick, t, alpha) {
  const skin = look.skin;
  const skinShade = look.skinShade;
  /* Body pitch follows the kick — whole torso dips and rises like a dolphin. */
  const bodyPitch = kick * 0.09;
  const hipY = kick * s * 2.2;
  const midY = kick * s * 5.5;
  const tipY = kick * s * 9.5;
  const flukeKick = kick * s * 11;

  ctx.save();
  ctx.rotate(bodyPitch);

  /* Trailing hair first (behind body) */
  const hairGrad = ctx.createLinearGradient(s * 8, -s * 6, -s * 36, s * 4);
  ...
  const hairFlow = Math.sin(t * 3.2 + kick) * s * 2;
  ctx.beginPath();
  ctx.moveTo(s * 10, -s * 4);
  ctx.quadraticCurveTo(s * 2 + hairFlow, -s * 14, -s * 12 + hairFlow * 0.6, -s * 10);
  ctx.quadraticCurveTo(-s * 28 + hairFlow, -s * 6, -s * 34, s * 2 + kick * s);
  ctx.quadraticCurveTo(-s * 22, s * 6, -s * 8, s * 2);
  ctx.quadraticCurveTo(s * 4, -s * 2, s * 12, s * 2);
  ctx.closePath();
  ctx.fill();
  ctx.beginPath();
  ctx.moveTo(s * 8, -s * 2);
  ctx.quadraticCurveTo(-s * 6, s * 8 + hairFlow * 0.4, -s * 24, s * 10 + kick * s * 0.5);
  ctx.quadraticCurveTo(-s * 12, s * 4, s * 6, s * 3);
  ctx.closePath();
  ctx.fill();

  /* Scaled tail — S-curve undulation from hips to fluke */
  const tailGrad = ctx.createLinearGradient(s * 2, 0, -s * 48, tipY);
  tailGrad.addColorStop(0, look.tail[0]);
  tailGrad.addColorStop(0.35, look.tail[1]);
  tailGrad.addColorStop(0.7, look.tail[2]);
  tailGrad.addColorStop(1, look.tail[3]);
  ctx.fillStyle = tailGrad;
  ctx.beginPath();
  ctx.moveTo(s * 2, hipY - s * 3);
  ctx.quadraticCurveTo(-s * 8, hipY + s * 5, -s * 20, midY + s * 2);
  ctx.quadraticCurveTo(-s * 32, tipY - s * 2, -s * 44, tipY);
  ctx.quadraticCurveTo(-s * 34, tipY + s * 8, -s * 22, midY + s * 7);
  ctx.quadraticCurveTo(-s * 10, hipY + s * 8, s * 2, hipY + s * 4);
  ctx.closePath();
  ctx.fill();
  /* Fluke — strong kick stroke */
  ctx.beginPath();
  ctx.moveTo(-s * 42, tipY);
  ctx.quadraticCurveTo(-s * 58, tipY - s * 12 + flukeKick * 0.15, -s * 52, tipY - s * 2);
  ctx.quadraticCurveTo(-s * 62, tipY + s * 10 + flukeKick * 0.1, -s * 44, tipY + s * 6);
  ctx.closePath();
  ctx.fill();
  ...
  for (let i = 0; i < 4; i++) {
    const lx = -s * (6 + i * 7);
    const ly = hipY + kick * s * (i * 1.8) + s * (2 + i);
    ctx.beginPath();
    ctx.moveTo(lx, ly);
    ctx.quadraticCurveTo(lx - s * 4, ly + s * 4 + kick * s, lx - s * 8, ly + s * 1.5);
    ctx.stroke();
  }

  /* Torso — slightly arched with the kick */
  const bodyGrad = ctx.createLinearGradient(s * 4, -s * 6, s * 4, s * 8);
  bodyGrad.addColorStop(0, skin);
  bodyGrad.addColorStop(1, skinShade);
  ctx.fillStyle = bodyGrad;
  ctx.beginPath();
  ctx.ellipse(s * 6, hipY * 0.35, s * 11, s * 7.5, 0.08 + kick * 0.04, 0, Math.PI * 2);
  ctx.fill();
  ...
  const drawClam = (ox, oy, tilt, flip) => {
    ...
    shell.addColorStop(0.35, look.shell[1]);
    shell.addColorStop(0.7, look.shell[2]);
    shell.addColorStop(1, look.shell[3]);
    ctx.fillStyle = shell;
    ctx.beginPath();
    ctx.moveTo(0, s * 2.6);
    ctx.quadraticCurveTo(-s * 5.2, s * 1, -s * 4.6, -s * 2);
    ctx.quadraticCurveTo(-s * 2.4, -s * 4.4, 0, -s * 3.5);
    ctx.quadraticCurveTo(s * 2.4, -s * 4.4, s * 4.6, -s * 2);
    ctx.quadraticCurveTo(s * 5.2, s * 1, 0, s * 2.6);
    ctx.closePath();
    ctx.fill();
    ctx.strokeStyle = "rgba(255, 250, 255, 0.45)";
    ctx.lineWidth = s * 0.35;
    for (let r = -2; r <= 2; r++) {
      ctx.beginPath();
      ctx.moveTo(0, s * 2.2);
      ctx.quadraticCurveTo(r * s * 1.3, 0, r * s * 2, -s * 2.6);
      ctx.stroke();
    }
    ctx.fillStyle = look.shellHi;
    ctx.beginPath();
    ctx.ellipse(0, -s * 1.8, s * 1.5, s * 1.1, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  };
  drawClam(s * 3, hipY * 0.2 - s * 1.5, -0.5, 1);
  drawClam(s * 9, hipY * 0.2 - s * 1.2, 0.45, -1);
  ctx.fillStyle = look.shellHi;
  ctx.beginPath();
  ctx.arc(s * 6, hipY * 0.2 - s * 0.6, s * 1, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = look.clasp;
  ctx.beginPath();
  ctx.arc(s * 6, hipY * 0.2 - s * 0.6, s * 0.45, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.fillStyle = skin;
  ctx.beginPath();
  ctx.ellipse(s * 16, -s * 1 + hipY * 0.15, s * 7.5, s * 8.2, 0.05, 0, Math.PI * 2);
  ctx.fill();

  /* Hair crown covering scalp + flowing bangs */
  ctx.fillStyle = hairGrad;
  ctx.beginPath();
  ctx.ellipse(s * 15, -s * 5 + hipY * 0.15, s * 9.5, s * 8, -0.15, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.moveTo(s * 10, -s * 6);
  ctx.quadraticCurveTo(s * 14, -s * 12, s * 20, -s * 8);
  ctx.quadraticCurveTo(s * 22, -s * 2, s * 18, s * 2);
  ctx.quadraticCurveTo(s * 14, -s * 2, s * 10, -s * 2);
  ctx.closePath();
  ctx.fill();
  ctx.globalAlpha = alpha * 0.5;
  ctx.fillStyle = look.hairHi;
  ctx.beginPath();
  ctx.ellipse(s * 17, -s * 7, s * 2.8, s * 5, 0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = alpha;
  ...
  ctx.beginPath();
  ctx.ellipse(s * 18.5, -s * 1.5, s * 1.05, s * 1.3, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(s * 14.8, -s * 1.7, s * 1, s * 1.25, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#fff";
  ctx.beginPath();
  ctx.arc(s * 18.8, -s * 1.9, s * 0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(s * 15.1, -s * 2.1, s * 0.28, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = look.lips;
  ctx.lineWidth = s * 0.5;
  ctx.beginPath();
  ctx.arc(s * 16.6, s * 2.2, s * 1.7, 0.15, Math.PI - 0.15);
  ctx.stroke();
...
function drawVagueMermaidSilhouette(now) {
  ...
  function swimX(progress, dir) {
    return dir > 0
      ? w * (-0.14 + progress * 1.28)
      : w * (1.14 - progress * 1.28);
  }

  function drawPasser(look, progress, dir, depth, sizeMul, alpha, phase) {
    const cx = swimX(progress, dir);
    const kick = Math.sin(t * 4.4 + phase + progress * 7);
    const cy =
      wy +
      wh * (depth + Math.sin(t * 0.9 + phase) * 0.025 + kick * 0.012);
    const s = dpr * sizeMul;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.scale(dir, 1);
    ctx.globalAlpha = alpha;
    drawSwimmingMermaidFigure(s, look, kick, t + phase, alpha);
    ctx.restore();
  }
...
const DUEL_LOBBY_TIMEOUT_MS = 35_000;
const DUEL_LOBBY_TIMEOUT_SEC = DUEL_LOBBY_TIMEOUT_MS / 1000;
const DUEL_LOBBY_POLL_MS = 350;
/** Base wait before hosting; each client also adds a small id-based stagger. */
const DUEL_LOBBY_CREATE_GRACE_MS = 200;
const DUEL_LOBBY_FETCH_LIMIT = 8;
...
/** Stagger lobby create so two friends tapping Cast off together don't both host on the same tick. */
function duelLobbyCreateStaggerMs() {
  const id = String(getDuelClientId() || "");
  let h = 2166136261;
  for (let i = 0; i < id.length; i++) {
    h ^= id.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  return Math.abs(h) % 900;
}

/**
 * When both players already hosted, only the newer lobby may join the older one.
 * Prevents mutual join (each becomes guest of the other) which orphans both into COM.
 */
function shouldJoinOpenLobbyWhileHosting(lobby, hostedMatchId, hostedCreatedAt) {
  if (!lobby?.matchId || lobby.matchId === hostedMatchId) return false;
  if (!hostedMatchId) return true;
  const lobbyMs = Date.parse(lobby.createdAt || "") || 0;
  const hostMs = Date.parse(hostedCreatedAt || "") || 0;
  if (lobbyMs && hostMs) {
    if (lobbyMs < hostMs) return true;
    if (lobbyMs > hostMs) return false;
  }
  return String(lobby.matchId) < String(hostedMatchId);
}
...
async function activateComDuelGuest(matchId) {
  /* Only fill COM while still an open lobby — never overwrite a real joiner. */
  const patchUrl =
    `${DUEL_MATCH_TABLE_URL}?id=eq.${encodeURIComponent(matchId)}` +
    `&status=eq.lobby&guest_client_id=is.null`;
  ...
  const rows = Array.isArray(body) ? body : [];
  if (rows[0]) return normalizeDuelMatchRow(rows[0]);
  /* Real player likely joined in the same window — use that match instead of COM. */
  const live = await safeFetchDuelMatchById(matchId);
  if (live?.guestClientId && !live.isComGuest) return live;
  throw new Error(`Duel COM fallback empty: ${matchId}`);
}
...
async function findOnlineMatch(matchKind, roundMs, deadlineMs, friendUserId = null) {
  ...
  const createAfterMs = startedAt + DUEL_LOBBY_CREATE_GRACE_MS + duelLobbyCreateStaggerMs();
  let backendReady = false;

  const clearHosted = () => {
    hostedMatchId = null;
    hostedCreatedAt = null;
    if (isCoop) coopLobbyMatchId = null;
    else duelLobbyMatchId = null;
  };

  const tryReturnJoined = async (joined) => {
  ...
  while (Date.now() < deadlineMs) {
    ...
    try {
      ...
      for (const lobby of joinCandidates) {
        ...
        if (!shouldJoinOpenLobbyWhileHosting(lobby, hostedMatchId, hostedCreatedAt)) continue;
        const joined = await tryJoinDuelLobby(lobby.matchId);
        if (joined) {
          const plan = await tryReturnJoined(joined);
          if (plan) return plan;
      ...
      if (!hostedMatchId && Date.now() >= createAfterMs) {
        ...
        if (created?.matchId) {
          ...
          hostedCreatedAt = created.createdAt || new Date().toISOString();
        ...
        } else if (row.createdAt && !hostedCreatedAt) {
          hostedCreatedAt = row.createdAt;
        }
      }
    } catch (err) {
  ...
  if (hostedMatchId) {
    ...
    try {
      ...
      /* activateCom may return a real PvP row if someone joined mid-fallback. */
      return planFromMatch(comRow, "host");
    } catch (err) {
      console.warn(err);
      try {
        const live = await safeFetchDuelMatchById(hostedMatchId);
        if (live?.guestClientId && !live.isComGuest) {
          if (isCoop) coopLobbyMatchId = null;
          else duelLobbyMatchId = null;
          return planFromMatch(live, "host");
        }
      } catch (_) {
        /* ignore */
      }
...
function musicWatchdogTick() {
  ...
  if (musicCtx.state === "suspended" || musicCtx.state === "interrupted") {
    ...
    void resumeMusicContext().then(() => {
      syncMusicMasterGain();
      restartSceneMusic(false);
    });
    return;
  }
  /* Near-silent gain used to look "fine" ( > 0 ) after a failed scene fade. */
  if (musicMasterGainValue() < MUSIC_AUDIBLE_GAIN_MIN) syncMusicMasterGain();
...
/** Below this, treat master gain as "silent" so the watchdog can recover. */
const MUSIC_AUDIBLE_GAIN_MIN = 0.04;
let sceneMusicSwitchTimer = 0;
...
function musicMasterGainValue() {
  if (!musicMaster) return 0;
  const v = Number(musicMaster.gain.value);
  return Number.isFinite(v) ? v : 0;
}

function setMusicMasterGainNow(target) {
  ...
  try {
    if (musicCtx && musicCtx.state !== "closed") {
      ...
      musicMaster.gain.setValueAtTime(value, now);
    } else {
      musicMaster.gain.value = value;
    }
  } catch {
    try {
      musicMaster.gain.value = value;
...
function syncMusicMasterGain() {
  if (!musicMaster) return;
  if (!musicEnabled) {
    setMusicMasterGainNow(0);
    return;
  }
  setMusicMasterGainNow(sceneMusicTargetGain());
}
...
/**
 * Switch beds without dipping master gain to silence (that left music stuck
 * inaudible when restarts raced or AudioContext was still suspended).
 */
function switchSceneMusic(startFn) {
  if (!musicEnabled || typeof startFn !== "function") return;
  unlockAudioFromGesture();
  if (sceneMusicSwitchTimer) {
    clearTimeout(sceneMusicSwitchTimer);
    sceneMusicSwitchTimer = 0;
  }
  stopAllSceneMusicTimers();
  syncMusicMasterGain();
  startFn(true);
  syncMusicMasterGain();
  sceneMusicSwitchTimer = window.setTimeout(() => {
    sceneMusicSwitchTimer = 0;
    if (!musicEnabled) return;
    if (!expectedMusicTimer()) restartSceneMusic(true);
    syncMusicMasterGain();
  }, 120);
  void resumeMusicContext().then(() => {
    if (!musicEnabled) return;
...
function startEventsMusic(forceRestart = true) {
  ...
  if (!forceRestart && eventsMusicTimer && musicContextRunning() && musicMasterGainValue() >= MUSIC_AUDIBLE_GAIN_MIN) {
    return;
  }
  unlockAudioFromGesture();
  ...
  syncMusicMasterGain();
  void resumeMusicContext().then(() => syncMusicMasterGain());
}
...
function startAdventureMusic(forceRestart = true) {
  ...
  if (!forceRestart && adventureMusicTimer && musicContextRunning() && musicMasterGainValue() >= MUSIC_AUDIBLE_GAIN_MIN) {
    return;
  }
  unlockAudioFromGesture();
  homeAudioUnlocked = true;
...
function startReefMusic(forceRestart = true) {
  ...
  if (!forceRestart && gameMusicTimer && musicContextRunning() && musicMasterGainValue() >= MUSIC_AUDIBLE_GAIN_MIN) {
    return;
  }
  stopAdventureMusic();
  unlockAudioFromGesture();
